...
# シーンを保存する
def save_scene(files, scene_point_dic):
    base = os.path.dirname(os.path.abspath(__file__))   # スクリプト実行ディレクトリ
    result_scene_path = os.path.normpath(os.path.join(base, r'temp\scene'))  # シーン保存ディレクトリ
    video_path = os.path.normpath(os.path.join(base, r'temp\cut'))    # 動画パス
    ...
    # 動画IDリストの作成
    # --------------------------------------------------
    video_id_list = [f.replace('.mp4', '') for f in os.listdir(video_path)]  # 

    # --------------------------------------------------
    # シーン分割
    # --------------------------------------------------
    for video_id in video_id_list:
        input_video_path = files[video_id_list.index(video_id)] # 動画ファイルの入力パス 
        #input_video_path = video_path + '\\' + video_id + '.mp4' # 動画ファイルの入力パス 
        #input_video_path = files # 動画ファイルの入力パス 
...
# シーンの統合
def scene_integration(files):
    base = os.path.dirname(os.path.abspath(__file__))   # スクリプト実行ディレクトリ
    cut_data_path = os.path.normpath(os.path.join(base, r'temp\merged_label.csv'))  # カットデータのパス
    scene_data_path = os.path.normpath(os.path.join(base, r'temp\result_label.csv'))  # カットデータのパス
    ...
    # シーンを動画として新たに保存
    save_scene(files, scene_point_dic)
