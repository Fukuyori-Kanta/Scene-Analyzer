...
def cut_img_generate(files, result_cut_img_path):
    """
    各カットからカット画像を作成する関数
    カット画像 ・・・ 物体認識に使用する画像
    ...
    # 保存先の中身を初期化
    # --------------------------------------------------
    dest_folder_create(result_cut_img_path)
    ...
    video_id_list = files  # 一覧から拡張子を削除
    ...
    for video_id in video_id_list:
        ...
        input_video_path = video_id # 動画ファイルの入力パス 

        # --------------------------------------------------
        # 保存先フォルダの作成
        # --------------------------------------------------
        video_name = os.path.splitext(os.path.basename(video_id))[0]    # 動画名(拡張子なし)
        dest_path = os.path.join(result_cut_img_path, video_name) # 各動画のカット画像作成結果の保存先
        #dest_path = result_cut_img_path # 各動画のカット分割結果の保存先
